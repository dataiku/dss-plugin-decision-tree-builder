...
class TreeParser(object):
    ...
    def __init__(self, model_handler):
        ...
        self.model = model_handler.get_clf()
    ...
    def parse_nodes(self, tree, df):
        ...
        for k, v in self.model_handler.get_per_feature().items():
            if v["role"] != "INPUT":
                continue
            if v["type"] == "NUMERIC":
                if v["role"] == "INPUT":
                    tree.features[k]["missing_handling"] = v["missing_handling"]
                    if v["missing_handling"] == "IMPUTE":
                        tree.features[k]["missing_impute_value"] = collector_data[k]["missing_impute_with_value"]
                root_node.treated_as_numerical.add(k)
            else:
                root_node.treated_as_numerical.discard(k)
        ...
        while ids:
            ...
            tree.add_split(node_id, split_parameters.feature, value, left_child_id, right_child_id)
